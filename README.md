...
The built-in `openai_responses` generator now defaults to a balanced multi-file proposal profile instead of a one-or-two-edit prompt. It sends an expanded target-root snapshot, supports `proposal_scope=conservative|balanced|broad` plus `max_operations=<n>` through `--generator-option` or the matching `AUTOHARNESS_OPENAI_*` environment variables, and still asks the model to keep all edits inside one coherent hypothesis.
